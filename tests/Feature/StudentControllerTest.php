...
it('can store student with image', function () {
    $admin = createUser();
    ...
    assertFileExists(storage_path('app/public/'.$image->hashName()));
});
...
it('can update student with image', function () {
    $admin = createUser();
    $student = Student::factory()->create();
    ...
    expect($student->name)->toBe($request->name)
        ...
        ->and($student->phone_number)->toBe($request->phone_number)
        ->and($student->admission_date)->toBe($request->admission_date)
        ->and($student->image)->toBe($image->hashName());
